...
function isCoreAfternoonTime(item) {
  return /^(mincha|arvit|maariv|shabbat ends|yom tov ends|tzeit|rabbenu tam|shkia|sunset)/i.test(item.label);
}

function parseAfternoonSection(lines) {
  return parseSection(lines, "SHABBAT AFTERNOON").filter(isCoreAfternoonTime);
}
...
function holidayGroupForItem(item) {
  ...
  if (isCoreAfternoonTime(item)) return "afternoon";
  return null;
}
