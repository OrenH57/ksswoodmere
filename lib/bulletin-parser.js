...
function findHeadingIndex(lines, heading) {
  ...
  return lines.findIndex((line) => compactHeading(line) === target);
}
...
function parseBulletinText(text) {
  ...
  const firstDateLine = lines.find((line) => /\b\d{1,2}\s+\w+\s+\d{4}\b/.test(line)) || "";
  const parshaLine = lines.find((line) => /^Parashat\s+/i.test(line)) || "";
  ...
  return {
    ...
    notes: {
      parsha: parshaLine.replace(/^Parashat\s+/i, "").trim(),
