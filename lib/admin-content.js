...
function cleanScheduleKey(value) {
  const key = cleanText(value, 60).toLowerCase();
  return /^(weekday-shacharit|sunday-shacharit|daily-mincha-arvit)$/.test(key) ? key : "";
}
...
function normalizeScheduleItems(items, limit = 16) {
  ...
  return items
    ...
    .map((item) => {
      ...
      const header = cleanText(item?.header, 80);
      const key = cleanScheduleKey(item?.key);
...
function normalizeScheduleLabels(input, previous = defaultContent.scheduleLabels) {
  return Object.fromEntries(
    Object.entries(defaultScheduleLabels).map(([key, fallback]) => {
      const hasLabel = input && Object.prototype.hasOwnProperty.call(input, key);
      return [key, cleanText(hasLabel ? input[key] : previous?.[key] || fallback, 80) || fallback];
    })
  );
}
...
function normalizeContent(input, previous = defaultContent) {
  ...
  return {
    ...
    scheduleLabels: normalizeScheduleLabels(input?.scheduleLabels, previous.scheduleLabels),
